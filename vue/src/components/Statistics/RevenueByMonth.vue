...
onMounted(() => {
  if (props.stats && props.stats.length > 0) {
    ...
    const revenues = [] // Receita para o eixo Y
    ...
    props.stats.forEach(item => {
      ...
      revenues.push(parseFloat(item.total_revenue)) // Converte as receitas para números
    ...
    chartOptions.value = {
      ...
      title: {
        text: 'Revenue by Month', // Título do gráfico
        style: {
      ...
      yAxis: {
        title: {
          text: 'Total Revenue (€)', // Título do eixo Y
          style: {
        ...
        labels: {
          ...
          formatter: function () {
            return this.value + '€'; // Formata os valores do eixo Y com o símbolo €
          }
        },
      ...
      series: [
        {
          name: 'Revenue', // Nome da série
          data: revenues, // Dados de receita
