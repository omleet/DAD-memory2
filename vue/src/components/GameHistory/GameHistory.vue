<template>
    <div class="bg-gray-100 p-6 rounded-lg shadow-lg relative">
      ...
      <div class="max-w-4xl mx-auto py-8">
        ...
        <!-- Games Table -->
        <div v-else>
          <table v-if="gameHistoryStore.games.data?.length > 0" class="game-history-table w-full table-auto text-gray-700 mb-8">
            <thead>
            ...
            <tbody>
              <tr v-for="game in gameHistoryStore.games.data" :key="game.id" class="border-b hover:bg-gray-50">
                ...
                <td class="px-4 py-2">
                  <span v-if="game.status === 'PE'">Pending</span>
                  <span v-if="game.status === 'PL'">In Progress</span>
                  <span v-if="game.status === 'E'">Ended</span>
                  <span v-if="game.status === 'I'">Interrupted</span>
                </td>
                <td class="px-4 py-2">{{ gameHistoryStore.formatTotalTime(game.total_time) }}</td>
                <td class="px-4 py-2">{{ game.total_turns_winner || '-' }}</td>
                <td class="px-4 py-2">{{ game.winner_user_id || '-' }}</td>
          ...
          <!-- Pagination Controls -->
          <div v-if="gameHistoryStore.games.total > gameHistoryStore.games.per_page" class="flex justify-between items-center mt-4">
            <button @click="changePage(gameHistoryStore.currentPage - 1)" :disabled="gameHistoryStore.currentPage === 1"
            ...
            <span class="text-gray-600">
              Page {{ gameHistoryStore.currentPage }} of {{ gameHistoryStore.games.last_page }}
            </span>
            <button @click="changePage(gameHistoryStore.currentPage + 1)" :disabled="gameHistoryStore.currentPage === gameHistoryStore.games.last_page"
              class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded-full focus:outline-none focus:ring-2 focus:ring-gray-500">
              Next
            </button>
          </div>
