<template>
  <div class="bg-gray-100 p-6 rounded-lg shadow-lg relative">
    ...
    <div class="max-w-4xl mx-auto py-8">
      <h2 class="text-3xl font-bold text-gray-900 mb-10 text-center">Leaderboard by fewest Moves</h2>
          ...
          <div class="space-y-4">
            ...
            >
              <div class="flex items-center w-full">
                ...
                <!-- Barra de jogadas com cores distintas para o 1º, 2º e 3º lugares -->
                <div class="relative w-1/2 bg-gray-200 h-4 rounded-full">
                  <div
                    :style="{ width: `${player.total_turns_percentage}%` }"
                    :class="{
              ...
              <p class="ml-4 text-lg font-semibold text-gray-700">{{ player.total_turns_winner }}</p>
            </div>
          </div>
        </div>
      </div>
...
// Função para calcular a porcentagem de jogadas para o gráfico de barras
...
const fetchLeaderboards = async () => {
  try {
    const response = await axios.get("/leaderboard-by-moves"); // Endpoint para o leaderboard por jogadas
    leaderboards.value = response.data;

    // Determinar o menor número de jogadas (minTurns) para usar como base para calcular a porcentagem
