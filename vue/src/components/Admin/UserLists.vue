<template>
    <div class="bg-gray-100 p-6 rounded-lg shadow-lg relative">
        <button  @click="router.push('/admintab')"
        ...
        <div class="max-w-4xl mx-auto py-8">
            ...
            <div class="mb-6">
                ...
                <div class="relative mb-4">
                    ...
                    />
                    <svg v-if="userStore.searchName" @click="clearSearchName" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-red-500 cursor-pointer hover:text-red-700">
      ...
      <tr v-for="user in userStore.users.filter(user => user.id !== currentUserId)" :key="user.id">
        ...
        <td class="p-2 text-xs md:text-sm">
          ...
          <button @click="toggleBlockUser(user)" :class="user.blocked ? 'bg-green-500' : 'bg-yellow-500'" class="text-white px-2 py-1 rounded ml-2">
            ...
            <!-- Pagination -->
            <div v-if="!userStore.isLoading" class="flex justify-center mt-4">
                <button @click="userStore.toPage(1)" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">First</button>
                <button @click="userStore.previousPage" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">Prev</button>

                <button
                    v-for="page in userStore.pages"
                    :key="page"
                    @click="userStore.toPage(page)"
                ...
                <button @click="userStore.nextPage" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">Next</button>
                <button @click="userStore.toPage(userStore.pages)" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">Last</button>
            </div>
        </div>
    </div>
</template>
...
const removeAccount = (id) => {
  userStore.alertDialog.open(
    () => removeAccountConfirmed(id), // Pass the id as argument
    'Are you sure?',
    'Cancel',
    `Yes, DELETE user: ${id}`,
...
const toggleBlockUser = (user) => {
  ...
  userStore.alertDialog.open(
    ...
    'Are you sure?',
    'Cancel',
    `Yes, ${action} user: ${user.id}`,
    `This action cannot be undone. This will ${action.toUpperCase()} the user profile.`
  )
}
